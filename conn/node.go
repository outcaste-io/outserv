...
func (n *Node) ReportRaftComms(whoIs string, closer *z.Closer) {
	if !glog.V(3) {
		return
	}
	...
	for {
		select {
		case <-ticker.C:
			ticks := atomic.LoadInt64(&n.ticks)
			out := atomic.SwapInt64(&n.heartbeatsOut, 0)
			in := atomic.SwapInt64(&n.heartbeatsIn, 0)
			glog.Infof("RaftComm: [%s | %#x] Ticks: %d Heartbeats out: %d, in: %d",
				whoIs, n.Id, ticks, out, in)
...
func (n *Node) Tick() {
	atomic.AddInt64(&n.ticks, 1)
	n.Raft().Tick()
}

func (n *Node) EnsureRegularityOfTicks(whoIs string, dur time.Duration, closer *z.Closer) {
	numTicksPerSecond := int64(time.Second / dur)
	ticker := time.NewTicker(time.Second)
	defer ticker.Stop()
	glog.Infof("%s | CheckRegularityOfTicks would check for %d ticks per second\n",
		whoIs, numTicksPerSecond)

	lastCount := atomic.LoadInt64(&n.ticks)
	for {
		select {
		case <-ticker.C:
			cur := atomic.LoadInt64(&n.ticks)
			if 2*(cur-lastCount) < numTicksPerSecond {
				glog.Warningf("%s | Ticks one second ago: %d. Now: %d. Not enough ticks\n",
					whoIs, lastCount, cur)
			}
			lastCount = cur
		case <-closer.HasBeenClosed():
