...
func filterStemmers(input analysis.TokenStream) analysis.TokenStream {
	if len(input) == 0 {
		return input
	}
	// get filter from concurrent cache so we dont recreate.
	filter, err := bleveCache.TokenFilterNamed("stemmer_porter")
	if err != nil {
		glog.Errorf("Error while filtering stems: %s", err)
		return input
	}
	if filter != nil {
